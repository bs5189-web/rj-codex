...
> 纠正结论（2026-07-16）：头像菜单和设置入口已经恢复，但“剩余用量”尚未接入真实钱包。服务端兼容接口当前返回占位值 `total_usage=0`、`hard_limit_usd=5`，桌面端因此错误显示 `100%` 和月度重置。OAuth 已能识别同一用户 `116`，服务端仍需按该用户读取 New API 的 `quota`、`used_quota` 和 `request_count`。

## 1. 本次交付结果

| 验收项 | 实际结果 | 状态 |
|---|---:|---|
| 头像菜单“剩余用量” | 当前显示 `100%`，`Aug 1` 重置 | 未通过，服务端占位值 |
| 当前登录人额度 | 兼容接口返回 `$5.00 / $0.00`；网页真实余额 `$1920.93`、历史消耗 `$79.07` | 未通过 |
| 剩余比例核算 | 网页钱包口径应为 `1920.93 / 2000 = 96.0465%` | 待服务端真实值 |
...
| 兼容 billing 接口 + 本地适配 | 协议可用、数据不可用 | 当前仅返回占位值 | 中 | 服务端填入真实钱包后才能解决 |
...
- 已知事实：当前 Codex OAuth token 能访问兼容 billing 接口，但接口返回 `total_usage=0`、`hard_limit_usd=5`。
- 已知事实：同一 token 的 `/oauth/userinfo` 返回 `sub=116`，令牌内账号标识为 `user-116`；说明身份映射信息已经存在。
- 已知事实：网页控制台同一用户显示余额 `$1920.93`、历史消耗 `$79.07`、请求数 `3120`；网页前端读取 `/api/user/self` 的 `quota`、`used_quota`、`request_count`。
- 已知事实：OAuth token 直接请求 `/api/user/self` 会收到业务未授权，即使增加 `New-API-User: 116` 也一样。
- 因此，旧文档将 `$5` 判断为真实月度额度是错误的；当前 UI 的 `100%` 不能作为真实用量验收结果。
...
| 真实头像菜单 | 当前显示 `100%`、`Aug 1`；与网页钱包不一致，未通过真实用量验收 |
