...
function ruizhiInit(){
  try{
    ...
    function syncRuijieUniApiKeyEnvFromConfig(source){
      const lines=tomlLines(source);
      const table=findTomlTable(lines,"[model_providers.ruijie-uniapi]");
      if(!table)return;
      for(let index=table.start+1;index<table.end;index+=1){
        if(tomlKey(lines[index])==="api_key"){
          const key=tomlValue(lines[index]);
          if(key)process.env.RUIJIE_UNIAPI_KEY=key;
          return;
        }
      }
    }
